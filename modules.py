...
def update_password(username, new_password):  # Yeh function password update karne ke liye hai
    try:
        connection=connect_db()  # Database se connection establish karte hain
        mycursor = connection.cursor()  # Cursor object banate hain
        hashed_password = generate_password_hash(new_password)  # Password ko hash karte hain
        mycursor.execute("UPDATE registration SET password=%s WHERE name=%s", (hashed_password, username))  # SQL query execute karte hain password update karne ke liye
...
def one_student(Enrollment):  # Yeh function ek student ki details fetch karne ke liye hai
    try:
        connection=connect_db()  # Database se connection establish karte hain
        mycursor = connection.cursor()  # Cursor object banate hain
        mycursor.execute("SELECT * FROM registration WHERE Enrollment_NO=%s", (Enrollment,))  # SQL query execute karte hain student details fetch karne ke liye
        s = mycursor.fetchone()  # Ek row fetch karte hain
        return list(s) if s else None  # Agar row milti hai to usko list mein convert karke return karte hain, nahi to None return karte hain
    except Error as e:  # Agar koi error aata hai to usko catch karte hain
...
def add_subject(subject_code, start_time, end_time, day):
    try:
        ...
        if connection:
            ...
            connection.commit()
            mycursor.execute("SELECT * FROM subject WHERE Subject_code=%s", (subject_code,))
            ...
            mycursor.execute("INSERT INTO schedule (subject_code, start_time, end_time, day) VALUES (%s, %s, %s, %s)", 
...
def get_subject_by_current_day_and_time():
    try:
        ...
        if connection:
            ...
            subject = mycursor.fetchone()
            if not subject:
                print(f"Error: No subject scheduled for {current_day} at {current_time}")
                return None
            s=subject[0]
            # mycursor.execute("SELECT * from subject where Subject_code=%s",(s,))
            mycursor.execute("SELECT Subject_name from subject where Subject_code=%s",(s,))
    ...
    try:
        ...
        if connection:
            ...
            if not student:
                ...
                return False
            mycursor.execute("SELECT * FROM attendance WHERE RFID=%s AND Date=%s", (RFID, timestamp))
            attendance = mycursor.fetchone()
            if attendance:
                print(f"Error: Attendance already marked for {RFID} on {timestamp}")
                return False
            mycursor.execute("SELECT * FROM schedule WHERE day=%s AND start_time <= %s AND end_time >= %s", (datetime.now().strftime('%A'), datetime.now().strftime('%H:%M:%S'), datetime.now().strftime('%H:%M:%S')))
            subject = mycursor.fetchone()
            if not subject:
                print(f"Error: No subject scheduled for {datetime.now().strftime('%A')} at {datetime.now().strftime('%H:%M:%S')}")
                return False
            mycursor.execute("INSERT INTO attendance (RFID, Date, Subject_code) VALUES (%s, %s, %s)", (RFID, timestamp, subject[0]))
            connection.commit()
